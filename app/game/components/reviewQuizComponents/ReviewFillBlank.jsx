...
const ReviewFillBlank = ({ questionNumber, questionData, playerAnswer, isCorrect }) => {
    return (
        <div className='w-full md:w-[70%] h-fit px-4 py-6 md:p-10 bg-white border border-zinc-200 rounded-lg flex flex-col gap-6 md:gap-10 relative'>
            ...
            <div className='flex flex-col gap-4 md:gap-6'>
                <button
                    ...
                    ${playerAnswer == questionData.correctAnswer ? 'border-green-500 bg-green-50/50' : 'border-red-500 bg-red-50/50'}
                    `}>

                    <div className={`min-h-6 min-w-6 md:min-h-8 md:min-w-8 text-white rounded-full flex justify-center items-center
                        ${playerAnswer == questionData.correctAnswer ? 'bg-green-500' : 'bg-red-500'}
                        `}>
                        {playerAnswer == questionData.correctAnswer ? <MdDone /> : <MdOutlineAdd className='rotate-45' />}
                    ...
                    <div className='w-full flex justify-between items-center'>
                        ...
                        <p className={`text-xs md:text-sm ${playerAnswer == questionData.correctAnswer ? 'text-green-500' : 'text-red-500'}`}>Your Answer</p>
                    </div>
                ...
                {!isCorrect && (
                    <button
                        className={`w-full h-fit py-3 px-2 md:px-4 md:py-4 border-1 border-zinc-200 rounded-lg flex justify-start items-center gap-2 md:gap-4 cursor-pointer transition-all duration-300`}>
