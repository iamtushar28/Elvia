...
const QuizCreation = () => {
    ...
    const methods = useForm({
        defaultValues: {
            questions: [] // Initialize an empty array for our questions
        }
    });
    ...
    // Use useFieldArray to manage the dynamic list of questions
    ...
    // --- Handlers for Adding and Submitting ---
    const handleAddQuiz = (type) => {
        // Use `append` to add a new question object to the form's state
        append({
            type, // for rendering and saving
            timeLimit,
            questionText: '',
            // Initialize options and correct answer based on type
            options: type === 'mcq' ? [{ optionText: '' }, { optionText: '' }] : [],
    ...
    const onSubmit = (data) => {
        // The `data` object now contains the complete list of questions
        console.log("All Quiz Data:", data.questions);
    };

    // --- Quiz Rendering Logic ---
    // This function now uses the `index` to correctly register form fields
    const renderQuiz = (quiz, number, index) => {
        ...
        const commonProps = {
            ...
            setValue,
            errors,
            index, // Pass the index to the child component
        };
